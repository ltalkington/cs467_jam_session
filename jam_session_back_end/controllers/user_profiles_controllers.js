...
async function createUserProfile(inserts, res) {
    const query =
        "INSERT INTO User_Profile (user_id, display_name, user_photo_link, location, instruments, experience, liked_genres, disliked_genres, portfolio_link, hourly_fee, availability, review_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?);";
    await db.pool.query(query, inserts, function (error, results, fields) {
        return db.returnCallback(error, results, fields, res);
    });
}
